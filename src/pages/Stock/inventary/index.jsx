...
const VisualizarInventarioFinal = () => {
  ...
  useEffect(() => {
      ...
      (snapshot) => {
        try {
          ...
          if (data) {
            ...
            setInventarioData({})
            setConnectionError("Nenhum dado encontrado no banco de dados")
          }
        } catch (error) {
    ...
    // Cleanup function
    return () => {
      off(inventarioRef, "value", unsubscribe)
    }
  }, [])
  ...
  const itensFiltrados = datasFiltradas.flatMap((data) =>
    Object.values(inventarioData[data] || {})
      .filter(
        (item) =>
          item.sku.toLowerCase().includes(busca.toLowerCase()) || item.nome.toLowerCase().includes(busca.toLowerCase()),
      )
      .map((item) => ({ ...item, dia: data })),
  )
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6 flex items-center justify-center">
        <Card className="w-full max-w-md shadow-lg border-0">
          <CardContent className="pt-8 pb-8 flex flex-col items-center gap-6">
            ...
            <Button onClick={() => window.location.reload()} className="bg-red-600 hover:bg-red-700 text-white px-6">
              Tentar Novamente
            </Button>
          </CardContent>
        </Card>
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => setIntervaloRapido(key)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Button
                ...
                disabled={itensFiltrados.length === 0 || isExporting}
      ...
      <div className="flex items-center justify-between bg-white rounded-lg shadow-sm border border-slate-200 px-6 py-4">
        <div className="flex items-center gap-4">
          <div className="text-sm text-slate-600">
            Mostrando <span className="font-semibold text-slate-900">{itensPagina.length}</span> de{" "}
            <span className="font-semibold text-slate-900">{itensFiltrados.length}</span> itens
          </div>
          {(startDate || endDate || busca) && (
                  ...
                  <div key={idx} className="p-6 hover:bg-slate-50 transition-colors duration-200">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {item.estoqueAnterior !== undefined && (
                          <Badge className={`${stockStatus.color} border flex items-center gap-1 px-3 py-1`}>
            ...
            {Array.from({ length: Math.min(5, totalPaginas) }, (_, i) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  className={paginaAtual === pageNum ? "bg-blue-600 hover:bg-blue-700" : "hover:bg-slate-50"}
                >
                  {pageNum}
                </Button>
              )
