...
import { Building2, MapPin, Phone, Mail, User, CreditCard, Settings, Save, Search, Home, Hash, MessageSquare, Edit, X } from "lucide-react"
...
export default function EditarFornecedor() {
  ...
  const [fornecedor, setFornecedor] = useState({cnpj: "", razaoSocial: "", cep: "", endereco: "", numero: "", bairro: "", municipio: "", uf: "", contato: "", telefone: "", 
    email: "", grupo: "", observacoes: "", formasPag: "", status: "Ativo",})

  useEffect(() => {
    if (location.state && location.state.fornecedor) {setFornecedor(location.state.fornecedor);setLoading(false)} 
    else {const db = getDatabase(); const fornecedorRef = ref(db, `CadastroFornecedores/${id}`);
      get(fornecedorRef).then((snapshot) => {
        if (snapshot.exists()) {setFornecedor(snapshot.val())} 
        else {toast.error("Fornecedor não encontrado"); navigate("/Visualizar_Fornecedores")}
        setLoading(false);
      })
    }
  ...
  const buscarEndereco = async (cep) => {
    ...
    if (cepLimpo.length === 8) {
      try {
        ...
        if (!data.erro) {setFornecedor((prev) => ({...prev, endereco: data.logradouro || "", bairro: data.bairro || "", municipio: data.localidade || "", uf: data.uf || "",}))
  ...
  const { name, value } = e.target; setFornecedor((prev) => ({ ...prev, [name]: value }))
  if (name === "cep") {buscarEndereco(value)}}
  const handleSelectChange = (name, value) => {setFornecedor((prev) => ({ ...prev, [name]: value }))}
  const handleSubmit = (e) => {
    e.preventDefault()
    const db = getDatabase()
    set(ref(db, `CadastroFornecedores/${id}`), fornecedor)
      .then(() => {toast.success("Fornecedor atualizado com sucesso!"); navigate("/Visualizar_Fornecedores")})
        ...
        <div className="max-w-5xl mx-auto">
          <Card className="shadow-xl border-0">
            <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg">
              <CardTitle className="text-2xl md:text-3xl font-bold text-center flex items-center justify-center gap-3"><Edit className="w-8 h-8" /> Carregando...</CardTitle></CardHeader>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <Card className="shadow-xl border-0">
          <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg">
            <CardTitle className="text-2xl md:text-3xl font-bold text-center flex items-center justify-center gap-3"><Edit className="w-8 h-8" />Editar Fornecedor</CardTitle></CardHeader>
              <CardContent className="p-6 md:p-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <InputMask mask="99.999.999/9999-99" value={fornecedor.cnpj} onChange={handleChange}>{(inputProps) => <Input {...inputProps} name="cnpj" className="h-11" />} </InputMask>
                      </div>
                      <div className="space-y-2">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <InputMask mask="99999-999" value={fornecedor.cep} onChange={handleChange}>{(inputProps) => <Input {...inputProps} name="cep" className="h-11" />}</InputMask>
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <InputMask mask="(99) 99999-9999" value={fornecedor.telefone} onChange={handleChange}>{(inputProps) => <Input {...inputProps} name="telefone" className="h-11" />}</InputMask>
                      </div>
                      <div className="space-y-2">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t">
                <Button type="submit" className="flex-1 h-12 bg-blue-600 hover:bg-blue-700 text-white font-medium"><Save className="w-4 h-4 mr-2" />Salvar Alterações</Button>
                <Button type="button" onClick={() => navigate("/Visualizar_Fornecedores")} variant="outline" className="flex-1 h-12 border-gray-300 hover:bg-gray-50"><X className="w-4 h-4 mr-2" />Cancelar</Button>
              </div>
            </form>
          </CardContent>
        </Card>
