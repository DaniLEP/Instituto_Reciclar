...
const Card = ({ to, imgSrc, imgAlt, title, external = false }) => {
  const content = (
    <>
      <img src={imgSrc} onError={(e) => (e.target.src = "/fallback.png")} className="mx-auto h-[100px] md:h-[120px] lg:h-[140px]" alt={imgAlt}/>
  ...
  if (external) {
    return (
      <div className="bg-[#F6F6F6] rounded-lg shadow-lg p-6 cursor-pointer transform hover:scale-105 transition-transform">
        <a href={to} target="_blank" rel="noopener noreferrer" aria-label={title}> {content} </a>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#F6F6F6] rounded-lg shadow-lg p-6 cursor-pointer transform hover:scale-105 transition-transform">
      <Link to={to} aria-label={title}>{content}</Link>
    </div>
  );
};
...
export default function Home() {
  ...
  if (!usuario || !usuario.funcao) {
    return ( <div className="flex justify-center items-center min-h-screen font-['Roboto'] text-[18px] text-[#333]"> Carregando... </div> );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#6a11cb] to-[#2575fc] text-white font-['Chakra_Petch'] text-[20px] pb-28">
      ...
      <div className="border-b border-white"><Header /> </div>
      {/* Conteúdo principal */}
      <main className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 p-4">
        {cardsPermitidos.length > 0 ? (
          cardsPermitidos.map((card, i) => (
            <Card key={i} to={card.to} imgSrc={card.img} imgAlt={card.title} title={card.title} external={card.external} />
          ))
        ) : (<div className="col-span-full text-center text-lg opacity-80">Nenhuma opção disponível.</div> )}
      </main>
      ...
      <footer className="fixed bottom-0 w-full flex justify-between items-center px-5 py-3 bg-gradient-to-br from-[#6a11cb] to-[#2575fc] border-t border-white text-white text-sm shadow-md z-10">
        ...
        <div className="text-[15px] leading-tight">Seja bem-vindo - {usuario.nome || "Usuário"}</div>
