  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await set(ref(database, 'novousuario/' + newUser.uid), {
        nome, email, funcao, uid: newUser.uid
      });
      if (currentUser) await auth.updateCurrentUser(currentUser);
      setSuccessMessage('Usuário criado com sucesso!'); setErrorMessage('');
      setNome(''); setEmail(''); setFuncao(''); setSenha(''); setConfirmSenha('');
    ...
    <h2 className="text-[40px] font-bold text-center text-gray-700 mb-4">Registro de Novo Usuário</h2>
      <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="E-mail" 
          className="w-full p-4 border border-gray-300 rounded-lg bg-gray-50 text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        {/* FUNÇÃO */}
        <Input type="text" value={funcao} onChange={(e) => setFuncao(e.target.value)} required placeholder="Função"
          className="w-full p-4 border border-gray-300 rounded-lg bg-gray-50 text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        {/* SENHA */}
        ...
        <div className="flex flex-col md:flex-row justify-between md:col-span-2 gap-4 mt-4">
          <Button type="submit" disabled={isLoading} onClick={handleSubmit}
            className="w-full md:w-auto px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold shadow-md" > {isLoading ? 'Carregando...' : 'Criar Conta'}</Button>
          <Link to="/Verificacao_Usuario">
          <Button type="button" className="w-full md:w-auto px-6 py-3 bg-gray-500 hover:bg-gray-600 text-white rounded-xl font-semibold shadow-md">Voltar</Button>
          </Link>
        </div>
      </form>
        {errorMessage && <p className="text-red-600 mt-4">{errorMessage}</p>} {successMessage && <p className="text-green-600 mt-4">{successMessage}</p>}
    </div>
  </div>
);
}