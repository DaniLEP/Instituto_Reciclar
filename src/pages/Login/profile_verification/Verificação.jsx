...
const Card = ({ to, imgSrc, imgAlt, title }) => (
  <div className="card bg-white  rounded-lg shadow-lg p-6 cursor-pointer transform hover:scale-105 transition-transform">
    <Link to={to} aria-label={title} className="flex flex-col items-center">
      <img src={imgSrc} className="h-16 md:h-20 lg:h-24" alt={imgAlt}/>
  ...
  const salvarAlteracoes = () => {
    if (editingUser) {
      update(ref(database, `novousuario/${editingUser.uid}`), {nome, funcao})
        .then(() => {alert("Usuário atualizado com sucesso!"); setEditingUser(null); setNome(""); setFuncao("");})
  ...
  return (
    <div className="min-h-screen p-5 flex flex-col items-center bg-gradient-to-r from-[#6a11cb] to-[#2575fc]">
        ...
        <tbody>
          {usuarios.map((usuario) => (
            <tr key={usuario.uid} className="bg-white">
              ...
              <td className="p-2 border-b"><Button onClick={() => {setEditingUser(usuario); setNome(usuario.nome); setFuncao(usuario.funcao);}} 
                className="px-3 py-1 bg-blue-500 text-white rounded-lg mr-2">Editar</Button>
                <Button onClick={() => excluirUsuario(usuario.uid)} className="px-3 py-1 bg-red-500 text-white rounded-lg">Excluir</Button>
              </td>
            </tr>
          ))}
        </tbody>
        ...
        {usuarios.map((usuario) => (
          <div key={usuario.uid} className="border rounded-lg p-4 mb-4 bg-white shadow-md">
            ...
            <div className="mt-2 flex justify-between">
              <Button onClick={() => {setEditingUser(usuario); setNome(usuario.nome); setFuncao(usuario.funcao);}} 
                className="px-3 py-1 bg-blue-500 text-white rounded-lg"> Editar</Button>
              <Button onClick={() => excluirUsuario(usuario.uid)} className="px-3 py-1 bg-red-500 text-white rounded-lg">Excluir</Button>
            </div>
          </div>
        ))}
      ...
      <div className="flex flex-wrap justify-center gap-5 mt-8">
        <Card to="/Registro_Usuario" imgSrc="/novo-usuario.png" imgAlt="Novo Usuário"  title="Novo Usuário" />
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white rounded-lg p-8 w-full max-w-md">
            ...
            <label className="block mb-4">Nome:
              <Input type="text" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full p-2 border rounded mt-2" />
            </label>
            <label className="block mb-4">Função:
              <Input type="text" value={funcao} onChange={(e) => setFuncao(e.target.value)} className="w-full p-2 border rounded mt-2" />
            </label>
            <div className="flex justify-between mt-6">
              <Button onClick={salvarAlteracoes}className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition">Salvar</Button>
              <Button onClick={() => setEditingUser(null)} className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition">Cancelar</Button>
            </div>
          </div>
        </div>)}
    </div>
